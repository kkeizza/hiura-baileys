...
const extractGroupMetadata = (result) => {
    ...
    const metadata = {
        ...
        participants: WABinary_1.getBinaryNodeChildren(group, 'participant').map(({ attrs }) => {
            const isLid = WABinary_1.isLidUser(attrs.jid);
            const pn = attrs.phone_number;
            const hasPn = WABinary_1.isPnUser ? WABinary_1.isPnUser(pn) : (pn && pn.endsWith('@s.whatsapp.net'));
            // Grup LID: id pakai PN agar bisa dicompare dengan m.sender
            // Grup PN: id tetap JID biasa
            return {
                id: isLid && hasPn ? pn : attrs.jid,
                jid: attrs.phone_number || attrs.jid,
                phoneNumber: isLid && hasPn ? pn : undefined,
                lid: isLid ? attrs.jid : (WABinary_1.isPnUser && WABinary_1.isPnUser(attrs.jid) && WABinary_1.isLidUser(attrs.lid) ? attrs.lid : undefined),
