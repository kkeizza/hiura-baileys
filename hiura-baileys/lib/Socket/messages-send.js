...
const HiuraAdvancedHandler = require('./hiura-advanced');
...
const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, cachedGroupMetadata, useCachedGroupMetadata, statusJidList, AI = true }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroup || isStatus) {
                ...
                // listMessage, buttonsMessage, templateMessage tidak render di grup.
                // Auto-convert ke interactiveMessage nativeFlowMessage.
                // (Ported from blckrose-baileys 2.0.7)
                // ──────────────────────────────────────────────────────────────
                let compatPatched = patched;
                if (isGroup && compatPatched) {
                    if (compatPatched.listMessage) {
                        const list = compatPatched.listMessage;
                        compatPatched = {
                            interactiveMessage: {
                                nativeFlowMessage: {
                                    buttons: [{
                    ...
                    } else if (compatPatched.buttonsMessage) {
                        const bMsg = compatPatched.buttonsMessage;
                        const btns = (bMsg.buttons || []).map(btn => ({
                            name: 'quick_reply',
                            buttonParamsJson: JSON.stringify({
                        ...
                        compatPatched = {
                            interactiveMessage: {
                                nativeFlowMessage: { buttons: btns, messageParamsJson: '', messageVersion: 1 },
                                body: { text: bMsg.contentText || bMsg.text || '' },
                                footer: bMsg.footerText ? { text: bMsg.footerText } : undefined,
                                contextInfo: bMsg.contextInfo
                            }
                        };
                    } else if (compatPatched.templateMessage) {
                        const tmpl = compatPatched.templateMessage.hydratedTemplate || compatPatched.templateMessage.fourRowTemplate;
                        if (tmpl) {
                            const hydratedButtons = tmpl.hydratedButtons || [];
                            const btns = hydratedButtons.map(hBtn => {
                ...
                const bytes = Utils_1.encodeWAMessage(compatPatched);
    ...
    const hiuraHandler = new HiuraAdvancedHandler(Utils_1, waUploadToServer, relayMessage);
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            // ── Auto-fix mentions: LID → s.whatsapp.net ──
            if (content && Array.isArray(content.mentions) && content.mentions.length) {
                content.mentions = content.mentions
                    .map(m => normalizeMentionJid(m))
                    .filter(Boolean);
            }
            // Juga fix di contextInfo kalau ada
            if (content && content.contextInfo && Array.isArray(content.contextInfo.mentionedJid)) {
                content.contextInfo.mentionedJid = content.contextInfo.mentionedJid
                    .map(m => normalizeMentionJid(m))
                    .filter(Boolean);
            }
         const { filter = false, quoted } = options;
         const getParticipantAttr = () => filter ? { participant: { jid } } : {};
         const messageType = hiuraHandler.detectType(content);
         ...
         if (messageType) {
             switch(messageType) {
                 case 'PAYMENT':
                     const paymentContent = await hiuraHandler.handlePayment(content, quoted);
                 ...
                 case 'PRODUCT':
                     const productContent = await hiuraHandler.handleProduct(content, jid, quoted);
                 ...
                 case 'INTERACTIVE':
                     const interactiveContent = await hiuraHandler.handleInteractive(content, jid, quoted);
                 ...
                 case 'ALBUM':
                     return await hiuraHandler.handleAlbum(content, jid, quoted)
                 case 'EVENT':
                     return await hiuraHandler.handleEvent(content, jid, quoted)
                 case 'POLL_RESULT':
                     return await hiuraHandler.handlePollResult(content, jid, quoted)
                 case 'GROUP_STORY':
                     return await hiuraHandler.handleGroupStory(content, jid, quoted)
             }
         }
