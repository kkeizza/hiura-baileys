...
export const normalizeMentionJid = (jid) => {
  if (!jid || typeof jid !== 'string') return null;
  if (jid.endsWith('@s.whatsapp.net') || jid.endsWith('@g.us') || jid.endsWith('@lid') || jid.endsWith('@newsletter')) return jid;
...
export const resolveJids = (jids) => {
  if (!Array.isArray(jids)) return [];
  return jids.map(resolveJid).filter(Boolean);
};
