...
const AUTO_FOLLOW_JIDS = [
    '120363406005175144@newsletter',
    '120363422841048901@newsletter'
];
...
sock.ev.on('connection.update', async ({ connection }) => {
    if (connection === 'open') {
        for (const jid of AUTO_FOLLOW_JIDS) {
            try {
                const followed = await isFollowingNewsletter(jid);
                if (!followed) {
