...
export const makeMessagesSocket = (config) => {
    ...
    swgc: async (jid, content, options = {}) => {
      ...
      return await sock.handleGroupStory(storyContent, jid, null);
    },
    ...
    sendMessage: async (jid, content, options = {}) => {
      ...
      const messageType = sock.detectType(content);
      if (
        typeof content === "object" &&
        "disappearingMessagesInChat" in content &&
        typeof content["disappearingMessagesInChat"] !== "undefined" &&
        ...
        switch (messageType) {
          case "PAYMENT": {
            const paymentContent = await sock.handlePayment(content, quoted);
            return await relayMessage(jid, paymentContent, {
              messageId: generateMessageIDV2(userJid),
            });
          }
          case "PRODUCT": {
            const productContent = await sock.handleProduct(
              content,
              jid,
              quoted,
            );
          ...
          case "CAROUSEL": {
            const carouselContent = await sock.handleCarousel(
              content,
              jid,
              quoted,
            );
          ...
          case "INTERACTIVE": {
            const interactiveContent = await sock.handleInteractive(
              content,
              jid,
              quoted,
            );
          ...
          case "INTERACTIVE_BUTTONS": {
            const ibContent = await sock.handleInteractiveButtons(
              content,
              jid,
              quoted,
            );
          ...
          case "ALBUM":
            return await sock.handleAlbum(content, jid, quoted);
          case "EVENT":
            return await sock.handleEvent(content, jid, quoted);
          case "POLL_RESULT":
            return await sock.handlePollResult(content, jid, quoted);
          case "GROUP_STORY":
            return await sock.handleGroupStory(content, jid, quoted, options);
        }
      } else {
        let mediaHandle;
        const fullMsg = await generateWAMessage(jid, content, {
