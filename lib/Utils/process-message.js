...
/**
 * Decrypt an encrypted event edit message (secretEncryptedMessage with EVENT_EDIT type)
 * @param encData encrypted payload + IV
 * @param ctx additional info required for decryption
 * @returns decoded Message
 */
export function decryptEventEdit({ encPayload, encIv }, { eventCreatorJid, eventMsgId, eventEncKey, responderJid }) {
    const sign = Buffer.concat([
        toBinary(eventMsgId),
        toBinary(eventCreatorJid),
        toBinary(responderJid),
 ...
 * @returns decoded Message
 */
export function decryptComment({ encPayload, encIv }, { commentCreatorJid, commentMsgId, commentEncKey, commentJid }) {
    const sign = Buffer.concat([
 ...
 */
export function decryptReaction({ encPayload, encIv }, { reactionCreatorJid, reactionMsgId, reactionEncKey, reactionJid }) {
    const sign = Buffer.concat([
...
const processMessage = async (message, { shouldProcessHistoryMsg, placeholderResendCache, ev, creds, signalRepository, keyStore, logger, options, getMessage }) => {
    ...
    if (protocolMsg) {
        ...
        if (proto.Message.SecretEncryptedMessage.SecretEncType[encEventEdit.secretEncType] !== 'EVENT_EDIT')
            return;
        const eventMsg = await getMessage(creationMsgKey);
        if (eventMsg) {
            try {
                ...
                if (!eventEncKey) {
                    ...
                    const editedContent = normalizeMessageContent(responseMsg);
                    const protocolMsg = editedContent?.protocolMessage;
                    ev.emit('messages.update', [
                        {
                            key: { ...message.key, id: protocolMsg?.key?.id },
                            update: {
        ...
        if (commentMsg) {
            ...
            logger?.warn({ encComment, creationMsgKey }, 'creation message not found, cannot decrypt comment');
        }
    }
        ...
        if (reactMsg) {
            ...
            logger?.warn({ encReaction, creationMsgKey }, 'creation message not found, cannot decrypt reaction');
        }
    }
